...
import pathlib as _pl  # for Gradio patch helpers
import tempfile as _tmp  # for Gradio patch helpers
import traceback as _tb  # for diagnostics
import inspect as _inspect  # handle async/sync originals safely
from typing import Any  # typing for wrappers
...
# ---- Gradio preprocessing diagnostics + safe suppression patch ----
# This patch silences recurring InvalidPathError when a directory path leaks into Gradio's
# preprocessing pipeline, while adding high-signal diagnostics to locate the source.
try:  # Guard import so tests without Gradio can still run
...
def _is_dirlike_path(p: Any) -> bool:
    """Return True if p exists and is a directory, else False. Safe for any input."""
    try:
        if not p:
            return False
        pp = _P(str(p))
...
if _pu is not None:
    ...
    async def _diag_move_to_cache(payload: Any, check_in_upload_folder: bool = True):
        """Diagnostic wrapper: logs when a directory path is seen in preprocessing.
        Mirrors Gradio signature; supports original being async or sync.
        """
        try:
        ...
        except Exception:  # never break the pipeline on diagnostics
            pass
        if callable(_orig_move_to_cache):
            try:
                ...
                # Do not hide original errors unrelated to our patch
                raise
        return payload

    def _safe_check_allowed(path: str, check_in_upload_folder: bool = True) -> None:
        """Allow directories and other rejected paths to pass check stage; later wrappers neutralize them safely.

        Additionally, if the original _check_allowed raises InvalidPathError, suppress it to avoid noisy logs.
        """
        try:
            ...
            return None
        except Exception as e:  # Suppress only Gradio InvalidPathError
            try:
    ...
    def _safe_hash_file(file_path: str) -> str:
        """Avoid hashing directories; return stable pseudo-hash for determinism."""
    ...
    def _safe_save_file_to_cache(file_path: str, cache_dir: str | None = None) -> str:
        """If a directory sneaks through, substitute a benign temp file to keep queue quiet."""
        if _is_dirlike_path(file_path):
            try:
                ...
                # Fall through to original if substitution fails
                pass
